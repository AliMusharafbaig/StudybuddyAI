...
async def token_a(user_a, client):
    response = await client.post("/api/auth/login", json={"email": "usera@example.com", "password": "passwordA"})
    return response.json()["access_token"]

@pytest.fixture
async def token_b(user_b, client):
    response = await client.post("/api/auth/login", json={"email": "userb@example.com", "password": "passwordB"})
    return response.json()["access_token"]
...
async def test_course_isolation(client: AsyncClient, token_a, token_b, db_session):
    ...
    response = await client.get("/api/courses", headers={"Authorization": f"Bearer {token_b}"})
    ...
    response = await client.get(f"/api/courses/{course_a_id}", headers={"Authorization": f"Bearer {token_b}"})
...
async def test_chat_isolation(client: AsyncClient, token_a, token_b):
    ...
    response = await client.get("/api/chat/history", headers={"Authorization": f"Bearer {token_b}"})
    assert response.status_code == 200
